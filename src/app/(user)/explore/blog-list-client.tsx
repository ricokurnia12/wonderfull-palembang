...
export function BlogListClient({ initialData, initialSearch = "", initialCategory }: BlogListClientProps) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            if (debouncedSearchQuery === initialSearch && selectedCategory === initialCategory) {
                ...
                <AnimatePresence mode="wait">
                    ...
                    >
                        {blogData.data.length > 0 ? (
                            blogData.data.map((post, index) => <BlogCard title={language === "id" ? post.title : post.english_title} excerpt={language === "id" ? post.excerpt : post.english_excerpt} key={post.ID} post={post} index={index} />)
                        ) : (
