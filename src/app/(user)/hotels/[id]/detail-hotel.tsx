...
export function HotelDetailClient({ hotel }: HotelDetailClientProps) {
    ...
    const description = language === "id" ? hotel.description : hotel.description_english
    ...
    const getMapEmbedUrl = (mapsUrl: string) => {
        // This is a simplified approach - in production, you might want to use Google Maps Embed API
        const placeId = mapsUrl.match(/maps\/place\/([^/]+)/)?.[1]
        if (placeId) {
            return `https://www.google.com/maps/embed/v1/place?key=YOUR_API_KEY&q=${encodeURIComponent(hotel.name + " " + hotel.address)}`
        }
        return `https://www.google.com/maps/embed/v1/search?key=YOUR_API_KEY&q=${encodeURIComponent(hotel.name + " " + hotel.address)}`
    }
                    ...
                    <div className="flex items-center justify-between">
                        <Link href="/">
                            <Button variant="ghost" className="hover:bg-gray-100">
                                ...
                                {language === "id" ? "Kembali ke Daftar Hotel" : "Back to Hotel List"}
                            </Button>
                        </Link>
                ...
                <section className="relative w-full h-[400px] md:h-[500px] rounded-2xl overflow-hidden mb-8 shadow-2xl">
                    ...
                    <div className="absolute bottom-0 left-0 right-0 p-6 md:p-8 z-20 text-white">
                        <div className="max-w-4xl">
                            ...
                            <h1 className="text-3xl md:text-4xl font-bold mb-3 leading-tight">{hotel.name}</h1>

                            <div className="flex items-start gap-2 mb-4">
                                <MapPin className="w-5 h-5 mt-0.5 flex-shrink-0 text-blue-300" />
                                <p className="text-lg opacity-90 leading-relaxed">{hotel.address}</p>
                            </div>

                            {hotel.phone && (
                                <div className="flex items-center gap-2">
                                    <Phone className="w-5 h-5 text-green-300" />
                                    <a href={`tel:${hotel.phone}`} className="text-lg hover:text-blue-300 transition-colors">
                                        {hotel.phone}
                                    </a>
                                </div>
                            )}
                    ...
                    <div className="lg:col-span-2 space-y-8">
                        ...
                        <Card className="shadow-lg border-0 relative">
                            <Link href={hotel.maps} target="_blank">
                                <Button variant={'outline'} className="absolute top-4 right-4"><Map />Lihat Rute</Button>
                            </Link>
                            <CardHeader>
                                <CardTitle className="text-2xl text-gray-800">
                                    ...
                                    <div className="space-y-4">
                                        <div>
                                            <h3 className="text-lg font-semibold text-gray-700 mb-1">
                                                {language === "id" ? "Di Sekitar Lokasi" : "In the Area"}
                                            </h3>
                                    ...
                                    <div className="space-y-4">
                                        <div className="flex justify-between items-center">
                                            <h3 className="text-lg font-semibold text-gray-700">
                                                {language === "id" ? "Fasilitas Utama" : "Main Facilities"}
                                            </h3>
                        ...
                        <Card className="shadow-lg border-0 sticky top-8">
                            ...
                            <CardContent className="space-y-6">
                                <div className="text-center p-6 bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl border border-blue-100">
                                    <div className="flex items-center justify-center gap-2 mb-2">{renderStars(hotel.star)}</div>
                                    <p className="text-sm text-gray-600 mb-4">
                                        {language === "id" ? "Hotel Bintang" : "Star Hotel"} {hotel.star}
                                    </p>
                                    <p className="text-sm text-gray-600">
                                        {language === "id" ? "Harga terbaik tersedia di Traveloka" : "Best prices available on Traveloka"}
                                    </p>
                                </div>
                                ...
                                <Button variant={'outline'} asChild className="w-full" size="lg">
                                    ...
                                    >
                                        ...
                                        {language === "id" ? "Pesan di Traveloka" : "Book on Traveloka"}
                                    </a>
                                </Button>
