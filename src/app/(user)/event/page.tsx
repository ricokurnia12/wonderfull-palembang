...
export default function EventsList() {
  ...
  const years = [...new Set(events.map((event) => new Date(event.date).getFullYear().toString()))]
  ...
  useEffect(() => {
    ...
    if (selectedProvinces.length > 0) {
      result = result.filter((event) => selectedProvinces.includes(event.province))
    }

    // Filter by years
    if (selectedYears.length > 0) {
      result = result.filter((event) => selectedYears.includes(new Date(event.date).getFullYear().toString()))
    }
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="mb-8">
        <div className="flex flex-wrap gap-2 mb-4">
          ...
          >
            Location
            {showProvinceFilter ? <ChevronUp className="h-4 w-4" /> : <ChevronDown className="h-4 w-4" />}
          </Button>
        ...
        {/* Active Filters */}
        {(selectedCategories.length > 0 || selectedProvinces.length > 0 || selectedYears.length > 0) && (
          <div className="flex flex-wrap gap-2 mb-4">
            ...
            {selectedProvinces.map((province) => (
              <Badge key={province} variant="secondary" className="px-3 py-1">
                {province}
                <button className="ml-2" onClick={() => toggleProvince(province)}>
                  ×
                </button>
              </Badge>
            ))}
      ...
      <Tabs defaultValue="grid" className="w-full">
        ...
        <TabsContent value="calendar" className="mt-0">
          <div className="bg-white rounded-lg shadow p-6">
            <p className="text-center text-gray-500 py-12">Calendar view is coming soon!</p>
          </div>
        </TabsContent>
      </Tabs>
    </div>
